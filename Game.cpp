...
string scrambleWord(string word) {
    ...
    } while (scrambled == word); // pastikan berbeda
...
vector<string> getWordsByLevel(int level) {
    if (level == 1) {
        ...
        return { "pemrograman", "perpustakaan", "pengembangan", "algoritmis", "berkebangsaan", "masyarakat", "organisasi", "kementrian", "pendakian" };
    }
}
...
void showLeaderboard() {
    ...
    for (auto& p : sortedScores) {
        cout << p.first << " - " << p.second << " poin" << endl;
    }
}
...
int main() {
    ...
    for (int i = 0; i < maxQuestions; ++i) {
        string original = words[rand() % words.size()];
        string scrambled = scrambleWord(original);

        cout << "\nKata ke-" << (i + 1) << ": " << scrambled << endl;
        cout << "Tebakanmu (waktu maksimal " << timeLimit << " detik): ";

        string guess;
        // Mulai timer dan input
        auto start = chrono::steady_clock::now();

        // Tampilkan timer hitung mundur di background (optional)
        // tapi input tetap blocking, jadi ini hanya info waktu saja.
        guess = "";
        thread timerThread([&]() {
        ...
        if (elapsed > timeLimit) {
            cout << "Waktu habis! Jawaban yang benar: " << original << endl;
        } else {
